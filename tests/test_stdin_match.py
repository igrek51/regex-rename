...
def test_match_files_from_stdin():
    with StdoutCap() as cap:
        # find -name '_*.mp3'
        shell("printf './res/2mp3/_1.mp3\n./res/2mp3/_2.mp3' | regex-rename '_(.*).mp3'")
    assert_multiline_match(cap.uncolor(), r'''
\[.*\] DEBUG matching regex pattern pattern=_\(\.\*\).mp3 replacement=None dry_run=True full_match=False recursive=False padding=None
